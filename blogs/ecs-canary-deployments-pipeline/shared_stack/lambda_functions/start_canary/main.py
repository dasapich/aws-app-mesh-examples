        ...
        if event["Protocol"].lower() == "http":
            spec = {
                "httpRoute": {
                    "action": {
                        "weightedTargets": [
                            {"virtualNode": current_vn, "weight": current_vn_weight},
                            {"virtualNode": new_vn, "weight": new_vn_weight},
                        ]
                    },
                    "match": {"prefix": "/"},
                    "retryPolicy": {
...
def lambda_handler(event, _context):
    ...
    if event["deployment"]["new_version"] == 1:
        LOGGER.info(
            "Detected its first revision of the app, hence settings weight to 100."
        )
        try:
            if event["Protocol"].lower() == "http":
                spec = {
            ...
            return {"new_vn_weight": 100}
        except ClientError as ex:
            LOGGER.error(
                "Exception occured during creating a route for initial deployment"
            )
            raise ex
    else:
        ...
        for entry in entries:
            if entry["weight"] == 100:
                ...
                if _perform_canary(
                    event, current_vn_weight, new_vn_weight, current_vn, new_vn
                ):
                    LOGGER.info(
                        "Performing the Canary, new virtualNode weight is %s",
                        new_vn_weight,
                    )
                    return {
                ...
                if entry["virtualNode"].endswith(event["Sha"]):
                    new_vn_weight = (
                        100
                        if (entry["weight"] + int(event["PercentageStep"])) > 100
                        else (entry["weight"] + int(event["PercentageStep"]))
                    )
                    new_vn = entry["virtualNode"]
                    current_vn_weight = (
                        0
                        if (
                            int(event["canary_results"]["current_vn_weight"])
                            - int(event["PercentageStep"])
                        )
                        < 0
                        else (
                            int(event["canary_results"]["current_vn_weight"])
                    ...
                    current_vn = (
                        ...
                        + "-"
                        + event["canary_results"]["current_vn_sha"]
                    )
                    if _perform_canary(
                        event, current_vn_weight, new_vn_weight, current_vn, new_vn
                    ):
